...
const Normal_japanese = () => {
  ...
  const game_start = () => {
    (async () => {
      //単語リスト取得
      await change_quetion(word_list[ans_num].eng_word);
  ...
  const word_btn_click = async (e) => {
    await loading_st();
    for (let i = 0; i < 4; i++) {
      word_array[i] = word_list[i].eng_word_mean;
    }
                  ...
                  <Box sx={{ margin: "10px", textAlign: "center" }}>
                    ...
                    >
                      <Typography sx={theme.L_Midiam_label}>
                        <span>{element.eng_word_mean}</span>
                      </Typography>
                    </Button>
                  </Box>
                );
...
export default Normal_japanese;
